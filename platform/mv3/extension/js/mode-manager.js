...
/*
    ...
    - Calls `primoSeed(userModes)` from ./primo-seed.js inside
      readFilteringModeDetails (after unserialise, before admin
      overrides) so build-time defaults from primo-defaults.json
      apply only to settings the user hasn't manually changed.
    See ./primo-seed.js for the reconciliation rules and
    ./primo-defaults.js (auto-generated by tools/primo-prebuild.mjs).
*/
...
import { primoSeed } from './primo-seed.js';
...
export async function readFilteringModeDetails(bypassCache = false) {
    ...
    // [Primo] Reconcile shipped defaults with what the user has stored.
    // Runs before admin overrides so admin policy still wins.
    const primoResult = await primoSeed(userModes);
    if ( primoResult.modesModified ) {
        await localWrite('filteringModeDetails', serializeModeDetails(userModes));
    }
